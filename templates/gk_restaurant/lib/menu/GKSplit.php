...
if (!defined ('_GK_SPLIT_MENU_CLASS')) {
	...
	class GKSplit extends GKMenuBase{
            ...
            if($check == true) {
                ...
                if (@$this->children[$pid]) {
					...
					foreach ($this->children[$pid] as $row) {
						...
						$this->genMenuItem( $row, $level, $pos, 0, false);
	    ...
	    function beginMenuItems($pid=0, $level=0){
	        if(!$level) echo "<ul>";
			else echo "<ul id=\"gkDropSub$pid\" class=\"gkSplit\">";
	    }
	    ...
	    function beginMenu($startlevel=0, $endlevel = 10){
	        if(!$startlevel) echo "<nav id=\"gkSplitMenu\" class=\"gkMenu\">";
	        else echo "<nav id=\"gkDropSub\">";			
	    }
