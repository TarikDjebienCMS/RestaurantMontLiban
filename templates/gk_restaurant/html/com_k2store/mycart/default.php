...
<div id="k2storeCartPopup">
		<?php endif; ?>
		<div class='componentheading'> <span><?php echo JText::_( "My Shopping Cart" ); ?></span> </div>
		<div class="k2store_cartitems">
            	...
            	?>
										<tr class="row<?php echo $k; ?>">
												<?php if($this->params->get('show_thumb_cart')) : ?>
												<td style="text-align: center;"><?php if(!empty($image)) : ?>
														<img src="<?php echo $image; ?>" alt="<?php echo $item->product_name; ?>" title="<?php echo $item->product_name; ?>" />
														<?php endif; ?></td>
												<?php endif; ?>
												<td><a href="<?php echo $link; ?>"> <?php echo $item->product_name; ?> <?php echo $item->product_id; ?> </a> <br/>
														<?php if (!empty($item->attributes_names)) : ?>
														<?php echo $item->attributes_names; ?> <br/>
														<?php endif; ?>
														<input name="product_attributes[<?php echo $item->cart_id; ?>]" value="<?php echo $item->product_attributes; ?>" type="hidden" />
														<?php if (!empty($item->product_sku)) : ?>
														<b><?php echo JText::_( "SKU" ); ?>:</b> <?php echo $item->product_sku; ?> <br/>
														<?php endif; ?>
														<?php echo JText::_( "Price" ); ?>: <?php echo K2StorePrices::number($item->product_price); ?></td>
												<td style="width: 50px; text-align: center;"><?php $type = 'text'; 
                       ...
                       ?>
														...
														<input name="original_quantities[<?php echo $item->cart_id; ?>]" type="hidden" value="<?php echo $item->product_qty; ?>" /></td>
												<td style="text-align: right;"><?php $subtotal = $subtotal + $item->subtotal; ?>
														<?php echo K2StorePrices::number($item->subtotal); ?></td>
												<td><a title="Remove Item" onclick="k2storeCartRemove(this, <?php echo $item->cart_id; ?>, <?php echo $item->product_id; ?>, 2)">
								...
								<tfoot>
										<tr class="cart_subtotal">
												<td colspan="<?php echo $colspan=($this->params->get('show_thumb_cart'))? 3:2 ?>" style="font-weight: bold;"><?php echo JText::_( "Subtotal" ); ?></td>
												<td colspan="1" class="cart_subtotal_price"><?php echo K2StorePrices::number($subtotal); ?></td>
												<td>&nbsp;</td>
												<td>&nbsp;</td>
										</tr>
								</tfoot>
						...
						<table width="100%">
								<tr>
										<td colspan="5"><input style="float: right;" type="submit" class="button" value="<?php echo JText::_('Update Quantities'); ?>" name="update" /></td>
								</tr>
								<tr>
										<td colspan="5" style="white-space: nowrap;"><b><?php echo JText::_( "Tax and Shipping Totals" ); ?></b> <br/>
												<?php
                    	...
                    	?></td>
								...
								<tr>
										<td colspan="4"><?php if (!empty($this->return)) { ?>
												[<a href="<?php echo $this->return; ?>"> <?php echo JText::_( "Continue Shopping" ); ?> </a>]
												<?php } ?></td>
										<td style="text-align: right;" nowrap><div style="float: right;"> [<a href="<?php echo $checkout_url; ?>"> <?php echo JText::_( "Begin Checkout" ); ?> </a>] </div></td>
								</tr>
						</table>
